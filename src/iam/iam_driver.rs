...
impl IamDriver {
    ...
    pub async fn load_access_rights(
        &self,
        client_alias_id: i32,
    ) -> Result<HashMap<String, AccessRightEntity>, sqlx::Error> {
        let access_rights_redis_key = format!("IAM_AR_{}", client_alias_id);
        ...
        let access_right_entities = sqlx::query_as!(
            AccessRightEntity,
            r#"
            WITH active_alias_id AS (
                SELECT client_alias_id AS caid
                FROM groups_clients
                WHERE client_alias_id = $1
            ),
            running_subscription AS (
                SELECT spl.subs_plan_name
                ...
                WHERE gc.client_alias_id = (SELECT caid FROM active_alias_id) 
            ),
            c_feature_rights AS (
                SELECT
                    permission,
                ...
                FROM (
                    SELECT fr.permission, fr.feature
                    FROM acl.feature_rights AS fr
                    UNION ALL
                    SELECT fgr.permission, fgr.feature
            ...
            c_subscription_rights AS (
                ...
                JOIN running_subscription AS crs 
